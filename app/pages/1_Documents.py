...
if 'doc_manager' not in st.session_state:
    try:
        ...
        activity_logger.log_interaction(
            f"DocumentsManager initialization error: {str(e)}\n{traceback.format_exc()}", "error")
        st.stop()
...
try:
    ...
    activity_logger.log_interaction(
        f"Index verification error: {str(e)}\n{traceback.format_exc()}", "error")
    st.stop()
            ...
            if documents:
                ...
                for idx, doc in enumerate(documents):
                    try:
                        ...
                        activity_logger.log_interaction(
                            f"Error parsing document {idx}: {str(e)}", "error")
                        continue
        ...
        if selected_idx is not None:
            ...
            with col1:
                ...
                with st.expander("ℹMétadonnées", expanded=True):
                    ...
                    with col_b:
                        st.write(
                            f"**Modèle d'embedding:** {metadata.get('embedding_model', 'N/A')}")
            ...
            with col2:
                ...
                if st.button("Supprimer ce document", type="secondary"):
                    try:
                        activity_logger.log_interaction(
                            f"Document deleted: {selected_doc['_id']}", "info")
        ...
        with st.spinner("Traitement en cours..."):
            try:
                ...
                try:
                    ...
                    if success:
                        activity_logger.log_interaction(
                            f"Document indexed successfully: {uploaded_file.name}", "info")
                        st.success("✅ Document traité et indexé avec succès!")
                        st.balloons()
                    else:
                        activity_logger.log_interaction(
                            f"Document indexing failed: {uploaded_file.name}", "error")
