# I will use an specific structured output agent to ensure the question is
# valid if not i will return the justifcation of the llm
...
class RewriterAgent:
    # Increased temperature for more diverse rewrites
    def __init__(
            self,
        ...
        try:
            ...
            if isinstance(response, dict) and 'structured_response' in response:
                ...
                self.activity_logger.log_interaction(
                    f"Invalid response structure from rewriter agent: {response}", "error")
                return RewriterAgentResponse(
                    **{"neededRewrite": False, "rewritten_question": "Invalid response structure"})
        except Exception as e:
